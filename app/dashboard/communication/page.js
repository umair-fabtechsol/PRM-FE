          ...
          {dummyUsers.map((user, index) => (
            <div
              ...
              className={`cursor-pointer flex items-center p-4 border-b ${
                selectedUserId === user.id ? 'bg-blue-500 text-white' : 'bg-gray-50 text-black'
              }`}
              onClick={() => setSelectedUserId(user.id)}
            >
              <img src='/images/image.jpg' alt={user.name} className='w-12 h-12 rounded-full mr-4' />
              <div>
                ...
                <p
                  ...
                  className={`text-sm ${selectedUserId === user.id ? 'text-white' : 'text-gray-600'}`}
                >
            ...
            {selectedUser ? (
              <>
                <div className='flex items-center'>
                  <img src='/images/image.jpg' alt={selectedUser.name} className='w-10 h-10 rounded-full mr-4' />
                  <div>
                    <h2 className='font-semibold text-black'>{selectedUser.name}</h2>
                  ...
                  <div className='w-3/5'>
                    <div
                      className={`p-3 rounded-lg ${
                        msg.sender === 'You' ? 'bg-blue-500 text-sm text-white' : 'bg-gray-100 text-sm text-black'
                      } text-left`}
                    >
                      {msg.text}
                    </div>
                    <p className={`text-xs text-gray-500 mt-2 ${msg.sender === 'You' ? 'text-right' : 'text-left'}`}>
